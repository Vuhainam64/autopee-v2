...
import { sortRoles, generateApiPatternOptions, filterApis } from '../utils/permissions.js'
...
export default function ApiTable({
  ...
  const apiPatternOptions = useMemo(
    () => generateApiPatternOptions(apis),
    [apis]
  )

  const filteredApis = useMemo(
    () => filterApis(apis, searchText, patternFilter),
    [apis, searchText, patternFilter]
  )
  ...
  const handleSelectByPattern = () => {
    ...
    const matching = filteredApis
      .filter((api) => api.endpoint?.toLowerCase().startsWith(pattern))
      .map((a) => a._id)
      ...
      <div className="overflow-x-auto">
          <Table
            ...
            pagination={{
              ...
              showTotal: (total, range) => `${range[0]}-${range[1]} của ${total} APIs`,
              responsive: true,
