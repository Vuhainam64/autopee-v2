    ...
    handleAsync(async (req, res) => {
      ...
      if (body.displayName !== undefined) {
        if (body.displayName !== null && body.displayName !== "" &&
          ...
          return res.status(400).json({success: false, error: {message: "displayName must be a string"}});
        }
      ...
      if (body.phone !== undefined) {
        if (body.phone !== null && body.phone !== "" && typeof body.phone !== "string") {
          return res.status(400).json({success: false, error: {message: "phone must be a string"}});
        }
        if (body.phone && body.phone.trim() !== "" && !/^[0-9]{10,11}$/.test(body.phone.trim())) {
          return res.status(400).json({success: false, error: {message: "phone must be 10-11 digits"}});
        }
      ...
      if (body.dateOfBirth !== undefined) {
        if (body.dateOfBirth !== null && typeof body.dateOfBirth !== "string") {
          return res.status(400).json({success: false, error: {message: "dateOfBirth must be a string or null"}});
        }
      ...
      if (body.gender !== undefined) {
        if (body.gender !== null && !["male", "female", "other"].includes(body.gender)) {
          return res.status(400).json({success: false, error: {message: "gender must be one of: male, female, other"}});
        }
      ...
      if (body.photoURL !== undefined) {
        if (typeof body.photoURL !== "string") {
          return res.status(400).json({success: false, error: {message: "photoURL must be a string"}});
        }
        validatedData.photoURL = body.photoURL || null;
      }

      const updatedProfile = await updateUserProfile(req.user.uid, validatedData);
...
router.post(
    ...
    handleAsync(async (req, res) => {
      ...
      if (!currentSessionId) {
        return res.status(400).json({success: false, error: {message: "Cannot determine current session"}});
      }
